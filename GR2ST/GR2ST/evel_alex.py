...
from dataset import HERDataset, TenxDataset
...
def build_loaders_inference():
    ...
    for example in examples:
        dataset = TenxDataset(
            image_path=f"/d/zhoujl/my_model/dataset/Alex_NatGen/{example}/{example}.tif",
            spatial_pos_path=f"/d/zhoujl/my_model/dataset/Alex_NatGen/{example}/spatial/tissue_positions_list.csv",
            reduced_mtx_path=f"/d/zhoujl/my_model/dataset/Alex_NatGen/preprocessed_expression_matrices/Alex/{example}/preprocessed_matrix.npy",
            barcode_path=f"/d/zhoujl/my_model/dataset/Alex_NatGen/{example}/filtered_count_matrix/barcodes.tsv.gz",
            precomputed_features_dir=f"/d/zhoujl/my_model/dataset/Alex_NatGen/preprocessed_data/precomputed_features/{example}",
            cell_type_path=f"/d/zhoujl/my_model/dataset/Alex_NatGen/spots_type/{example}/summary_spot_types.tsv",
            train=False
        )
        loader = DataLoader(dataset, batch_size=len(dataset), shuffle=False, num_workers=0)
        loaders.append(loader)
    print("Finished building Tenx loaders")
    return loaders, examples
...
spot_expressions = [np.load(f"/d/zhoujl/my_model/dataset/Alex_NatGen/preprocessed_expression_matrices/Alex/{name}/preprocessed_matrix.npy")
                    for name in names]
datasize = [mat.shape[1] for mat in spot_expressions]

test_loaders, examples = build_loaders_inference()
...
for fold in range(6):
    ...
    model_path = f"./model_result_202512/10x/{names[fold]}/best_{fold}.pt"
    ...
    dir = f"./alex_pred_att/{names[fold]}/"
    os.makedirs(dir, exist_ok=True)
    np.save(f"./alex_pred_att/{names[fold]}/" + "matched_spot_expression_pred_202512.npy", final_pred.T)
    true = expression_gt
    pred = final_pred

    slice_name = names[fold]
    slice_dir = os.path.join("./st_results_202512/alex/")
    os.makedirs(slice_dir, exist_ok=True)

    gene_list_path = "/d/zhoujl/my_model/dataset/Alex_NatGen/Alex.npy"
