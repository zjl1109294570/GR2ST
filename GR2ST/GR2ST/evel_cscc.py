...
def save_embeddings(model_path, save_path, datasize, test_loaders):
    model = GR2ST(
            temperature=1.0,
            image_dim=1024,       
            spot_dim=171,         
    ...
    img_embeddings_all, spot_embeddings_all = get_embeddings(model_path, model,test_loaders)
...
if SAVE_EMBEDDINGS:
    ...
    for fold in range(12):
        save_embeddings(model_path=f"model_result/cscc/{names[fold]}/best_{fold}.pt",
                        ...
                        datasize=datasize, test_loaders=test_loaders)
...
for fold in range(12):
    ...
    gene_list_path = "../dataset/cscc/skin_hvg_cut_1000.npy"
    gene_list = list(np.load(gene_list_path, allow_pickle=True))
    
 
    slice_name = names[fold]  
    slice_dir = os.path.join("./st_results/cscc/")
