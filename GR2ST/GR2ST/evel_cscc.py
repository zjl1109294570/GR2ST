...
def build_loaders_inference():
    loaders = []
    print("Building loaders for all 12 slices (CSCC)...")
    for i in range(12):
...
spot_expressions = [np.load(f"./data/preprocessed_expression_matrices/cscc/{name}/preprocessed_matrix.npy")
                    for name in names]
...
for fold in range(12):
    ...
    model_path = f"./model_result_202512/cscc/{names[fold]}/best_{fold}.pt"
    if not os.path.exists(model_path):
        model_path = f"./model_result/cscc/fold{fold}/best_model.pt"
        if not os.path.exists(model_path):
            print(f"Model not found for fold {fold}, skipping...")
            all_heg_pcc_list.append(0)
            all_hvg_pcc_list.append(0)
            all_mse_list.append(0)
            all_mae_list.append(0)
            continue
    ...
    slice_dir = os.path.join("./st_results_202512/cscc/")
    os.makedirs(slice_dir, exist_ok=True)

    gene_list_path = "./data/skin_hvg_cut_1000.npy"
...
for i in range(len(all_hvg_pcc_list)):
    print(f"Fold {i} ({names[i]}): PCC={all_hvg_pcc_list[i]:.4f}, MSE={all_mse_list[i]:.4f}, MAE={all_mae_list[i]:.4f}")